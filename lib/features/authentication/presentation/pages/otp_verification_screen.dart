...
class OtpVerificationScreen extends StatefulWidget {
  ...
  // Optional: allow injecting a bloc instance (useful for tests or
  // when pushing the screen via Navigator where the ancestor provider may not be
  // visible to the new route). If provided, the screen will use this bloc via
  // BlocProvider.value.
...
class _OtpVerificationScreenState extends State<OtpVerificationScreen> {
  ...
  Widget build(BuildContext context) {
    // Require the AuthBloc to be provided by the caller. Tests already wrap this
    // screen with a BlocProvider.

    final body = BlocConsumer<AuthBloc, AuthState>(
      // allow passing an explicit bloc (useful when the route doesn't inherit
      // the provider). If null, BlocConsumer will look up the bloc in context.
